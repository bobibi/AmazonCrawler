...
class ReviewSpider(scrapy.Spider):
    ...
    url_template = \
    'http://www.amazon.com/gp/product-reviews/<<ASIN>>/ref=cm_cr_pr_viewopt_srt?ie=UTF8&showViewpoints=1&sortBy=recent&reviewerType=all_reviews&formatType=all_formats&filterByStar=all_stars&pageNumber=<<PAGE>>'
    ...
    def parse(self, response):
        ...
        for wrapper_selector in wrapper_selector_list:
            try:
                ...
                if wrapper:
                    extract_result = [html_extractor.extract(w, extractor_list, self.name, self.asin) for w in wrapper]
